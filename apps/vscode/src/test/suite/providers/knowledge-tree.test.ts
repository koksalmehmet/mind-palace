...
import { KnowledgeTreeProvider, KnowledgeTreeItem, KnowledgeItemType } from '../../../providers/knowledgeTreeProvider';
...
describe('KnowledgeTreeProvider Tests', () => {
  ...
  describe('Ideas Handling', () => {
    it('should fetch and display ideas', async () => {
      const mockIdeas = [
        ...
        { id: '2', content: 'Second idea', status: 'exploring', scope: 'room', scopePath: 'src/' },
      ];
    ...
    it('should group ideas by status', async () => {
      const mockIdeas = [
        ...
        { id: '2', content: 'Implemented idea', status: 'implemented', scope: 'palace' },
      ];
    ...
    it('should create idea items with commands', async () => {
      ...
      if (statusGroups.length > 0) {
        ...
        if (ideas.length > 0) {
          ...
          expect(ideas[0].command?.command).to.equal('mindPalace.showKnowledgeDetail');
        }
      }
    });
  });
  ...
  describe('Decisions Handling', () => {
    it('should fetch and display decisions', async () => {
      const mockDecisions = [
        { id: '1', content: 'Use TypeScript', status: 'active', scope: 'palace' },
      ];
      ...
      const categories = await provider.getChildren();
      const decisionsCategory = categories.find(item => item.label === 'Decisions');
  ...
  describe('Learnings Handling', () => {
    it('should fetch and display learnings', async () => {
      const mockLearnings = [
        { id: '1', content: 'Important learning', confidence: 0.95, scope: 'file', scopePath: 'src/main.ts' },
      ];
      ...
      const categories = await provider.getChildren();
      const learningsCategory = categories.find(item => item.label === 'Learnings');
  ...
  describe('TreeItem Creation', () => {
    it('should create items with correct collapsible state', () => {
      ...
      expect(categoryItem.collapsibleState).to.equal(vscode.TreeItemCollapsibleState.Collapsed);
    });
